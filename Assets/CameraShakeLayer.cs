...
public abstract class CameraShakeLayer : MonoBehaviour
{
    ...
    public float Duration
    {
        get
        {
            return Mathf.Max(useX ? xCurve.keys[xCurve.length - 1].time : 0f, useY ? yCurve.keys[yCurve.length - 1].time : 0, useZ ? zCurve.keys[zCurve.length - 1].time : 0) / playbackSpeed;
        }
    }
